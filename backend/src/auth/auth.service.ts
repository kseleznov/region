...
import { BCRYPT_ROUNDS, JWT_EXPIRY } from './auth.constants';
  ...
  private async getTokens(userId: number, email: string, name: string) {
    const [accessToken, refreshToken] = await Promise.all([
      this.jwtService.signAsync(
        ...
        { secret: this.accessSecret, expiresIn: JWT_EXPIRY.ACCESS },
      ),
      this.jwtService.signAsync(
        { sub: userId, email },
        { secret: this.refreshSecret, expiresIn: JWT_EXPIRY.REFRESH },
      ),
    ]);
