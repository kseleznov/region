...
export function RankDetailSheet({
  isOpen,
  onClose,
  userProgress,
}: RankDetailSheetProps) {
  const { t } = useTranslation();
  const { currentRank, nextRank, progressPercent, placesToNextRank } =
    getRankProgress(userProgress.placesVisited);
        ...
        <div className="text-center mb-8 pt-2">
          <div className="relative inline-flex items-center justify-center w-32 h-32 mb-5">
            ...
            <div className="absolute inset-5 flex items-center justify-center">
              <RankGlyph rank={currentRank} size="lg" color={SHEET_COLOR} />
            </div>
          </div>

          <h2
            className="text-3xl font-black mb-1"
            style={{ color: SHEET_COLOR }}
          >
          ...
          {nextRank ? (
            <p className="text-gray-600 text-sm">
              {t("ranks.moreToNext", { count: placesToNextRank })}{" "}
              <span className="font-bold text-brand-purple">
                {t(`ranks.${nextRank.key}` as TranslationKey)}
              </span>
            </p>
          ) : (
